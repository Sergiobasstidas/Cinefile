<template>
  <v-row justify="start" align-items="start">
    <v-col cols="12" lg="8" order-lg="1" order="2">
      ...
      <div v-if="hasComments" class="comentarios">
        <div v-for="(comentario, $index) in getComentarios" :key="$index">
          <div
            class="box-comment pa-6"
            :data-comment-id="$index"
            :data-comment-user="comentario.id"
          >
            <v-row
              justify="start"
              align-items="center"
      ...
      <p v-if="!isLogged && hasComments">
        <a href="/login">Ingresa a tu cuenta</a> para comentar esta película. Si
        aún no tienes una cuenta, puedes crearla <a href="/register">aquí</a>.
      </p>
    </v-col>
  ...
  export default {
    ...
    methods: {
      ...
      getLinkedLike(idComment) {
        ...
        let isLiked = comentarios[idComment].likes.filter(
          (like) => like == userActive
        );
      ...
      getLinkedDislike(idComment) {
        let comentarios = this.$store.getters["comments/getComments"];
        let userActive = this.$store.getters["comments/getUserActive"];
        let isDisliked = comentarios[idComment].dislikes.filter(
          (dislike) => dislike == userActive
        );
