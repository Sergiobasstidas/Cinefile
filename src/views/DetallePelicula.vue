  ...
  <div>
    <div class="bodyMovie">
      ...
      <div class="infografia">
        <v-container class="detalle_container">
          <h2 class="infografia__title">
            {{ getTitle }}<br /><small>{{ getOriginalTitle }}</small>
          </h2>
          <v-row
            align="center"
            justify="start"
          ...
          <div class="infografia__container">
            ...
            <div class="infografia__trailer">
              ...
              <v-layout align-center justify-space-between>
                <div class="infografia__valoracion d-flex">
                  <v-icon color="#2F80ED" size="25px">{{
                    getListas[0].icono
                  }}</v-icon>
                  <h4>{{ getListas[0].text }}</h4>
                </div>
                <div class="infografia-listas">
                  <addToList
                    class="agregar_Lista"
                    :movie="$store.state.infoMovie"
                  />
                </div>
              </v-layout>
            </div>
          ...
          <div class="infografia__genre">
            <h3 class="title_text mb-4">Géneros</h3>
            <v-chip
          ...
          <div class="infografia__share">
            <h3 class="title_text mb-4">Compartir</h3>
            <v-chip
              class="mr-12 infografia__chip"
              color="#3B5998"
              text-color="#fff"
              :href="`https://www.facebook.com/sharer/sharer.php?u=https%3A//www.cinefile.cl/${this.$route.params.type}/${this.$route.params.id}`"
              target="_blank"
              ><v-icon>mdi-facebook</v-icon> Compartir</v-chip
            >
            <v-chip
              class="mr-12"
              color="#55ACEE"
              text-color="#fff"
              :href="`https://twitter.com/intent/tweet?text=https%3A//www.cinefile.cl/${this.$route.params.type}/${this.$route.params.id}`"
              target="_blank"
              ><v-icon>mdi-twitter</v-icon> Compartir</v-chip
            >
            <v-chip
              class="mr-12"
              color="#00BB2D"
              text-color="#fff"
              :href="`whatsapp://send?text=https%3A//www.cinefile.cl/${this.$route.params.type}/${this.$route.params.id}`"
              target="_blank"
              ><v-icon>mdi-whatsapp</v-icon>Compartir</v-chip
            >
          </div>
          <Comentarios
            class="comentarios"
